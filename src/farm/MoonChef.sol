...
contract MoonChef is Ownable, ReentrancyGuard {
    ...
    function pendingTokens(
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 moonReward = multiplier
                .mul(moonPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accMoonPerShare = accMoonPerShare.add(
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accMoonPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeMoonTransfer(msg.sender, pending);
